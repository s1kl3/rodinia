...
CUDA: CUDA_BIN_DIR
	...
	cd cuda/myocyte;		make;	cp myocyte.out $(OPENCL_BIN_DIR)
...
OPENCL: OPENCL_BIN_DIR
	...
	cd opencl/hotspot3D;		make;	cp 3D $(OPENCL_BIN_DIR)
	...
	cd opencl/lud;				make;	cp ocl/lud $(OPENCL_BIN_DIR)
...
CUDA_clean:
	if test -d $(CUDA_BIN_DIR); then cd $(CUDA_BIN_DIR); rm -f *; fi
	for dir in $(CUDA_DIRS) ; do cd cuda/$$dir ; make clean ; cd ../.. ; done
	
OMP_clean:
	if test -d $(OMP_BIN_DIR); then cd $(OMP_BIN_DIR); rm -f *; fi
